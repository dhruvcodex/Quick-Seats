...
public class ChkUser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       String email=request.getParameter("uname");
        ...
        ResultSet rs = smt.executeQuery("select * from user where email = '"+email+"' and password = '"+pass+"' ");
        if(rs.next()){
            HttpSession hs = request.getSession(true);
            hs.setAttribute("user_name", rs.getString("name"));
            hs.setAttribute("user_email", email);
            RequestDispatcher rd = request.getRequestDispatcher("Delbus");
            rd.forward(request, response);
        }
        else{
            out.print("<script>alert('Inavalid Email or Password !')</script>");
